...
const NavBar = (props) => {
  ...
  const navigation = props.currentUserId ? (
    <Grid container spacing={3} style={{ 'justify-content': 'space-between' }} id='navbar-items'>
      <Grid item xs={10}>
  ...
  const mobileNavigation = props.currentUserId ? (
    <div className='mobile-nav-overlay' style={{ height: '20%', visibility: 'hidden' }}>
      <NavLink
        style={{ color: "white" }}
        to={`/profile/${props.currentUserId}`}
        onClick={toggleNav}
    ...
    <>
      <AppBar position="fixed" className={classes.root}>
        <Toolbar style={{ justifyContent: 'center' }}>
          ...
          <NavLink style={{ color: "white" }} to="/" className='mobile-home-btn'>
            <Button color="inherit">Instantelegram</Button>
          </NavLink>
          {navigation}
        </Toolbar>
